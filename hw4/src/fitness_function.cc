...
SgdTestDataCost::SgdTestDataCost(std::unique_ptr<IDataSupplier>&& data_supplier)
    : data_supplier_(std::move(data_supplier)) {}

double SgdTestDataCost::Assess(const IChromosome& chromosome) const {
  const auto kit = static_cast<const SgdKit&>(chromosome);
...
SgdNagTestDataCost::SgdNagTestDataCost(
    std::unique_ptr<IDataSupplier>&& data_supplier)
    : data_supplier_(std::move(data_supplier)) {}

double SgdNagTestDataCost::Assess(const IChromosome& chromosome) const {
  const auto kit = static_cast<const SgdNagKit&>(chromosome);
  ...
  auto metrics = perceptron.SgdNag(train_data, test_data, cfg, kit.get_gamma());
  return std::exp(1 / (metrics.test_cost.back() + 1e-8));
}
